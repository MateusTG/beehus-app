...
export default function Credentials() {
    ...
    const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; id: string; label: string }>({
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowModal(false);
            setFormData({ workspace_id: '', label: '', username: '', password: '', metadata: {} });
            setEditingId(null);
            fetchCredentials();
        } catch (error) {
            showToast(editingId ? 'Error updating credential' : 'Error creating credential', 'error');
            console.error(error);
        } finally {
            setCreating(false);
        }
    ...
    const confirmDelete = async () => {
        try {
            await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/credentials/${deleteModal.id}`);
            ...
            <div className="p-8 max-w-7xl mx-auto space-y-8">
                <header className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-400">Manage secure credentials for scraping jobs</p>
                    </div>
                    <button
                        onClick={() => {
                            setEditingId(null);
                                    ...
                                    <tr key={cred.id} className="hover:bg-white/5 transition-colors">
                                        <td className="px-6 py-4">
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 font-mono">#{cred.id.slice(0, 8)}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-slate-300 font-mono text-sm">{cred.username}</td>
                                        <td className="px-6 py-4 text-slate-300">{getWorkspaceName(cred.workspace_id)}</td>
                ...
                {showModal && (
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="glass p-8 rounded-xl border border-white/10 w-full max-w-lg">
                            ...
                            <form onSubmit={handleCreate} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    <p className="text-xs text-slate-500 mt-1">Friendly name to identify this credential</p>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-400 mb-2">
                                        Password {editingId && <span className="text-xs text-slate-500">(leave blank to keep current)</span>}
                                    </label>
                                    <input
                                        type="password"
                                ...
                                <div className="border-t border-white/10 pt-4">
                                    <h4 className="text-sm font-semibold text-slate-300 mb-3">Banking Information (Optional)</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-slate-400 mb-2">Agência (AG)</label>
                                            <input
                                                type="text"
                                                value={formData.metadata?.agencia || ''}
                                                onChange={(e) => setFormData({ 
                                                    ...formData, 
                                                    metadata: { ...formData.metadata, agencia: e.target.value }
                                                })}
                                                className="w-full bg-dark-surface border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-brand-500"
                                                placeholder="0001"
                                            />
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-slate-400 mb-2">Conta Corrente (CC)</label>
                                            <input
                                                type="text"
                                                value={formData.metadata?.conta_corrente || ''}
                                                onChange={(e) => setFormData({ 
                                                    ...formData, 
                                                    metadata: { ...formData.metadata, conta_corrente: e.target.value }
                                                })}
                                                className="w-full bg-dark-surface border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-brand-500"
                                                placeholder="12345-6"
                                            />
                                    ...
                                    <p className="text-xs text-slate-500 mt-2">
                                        These fields are optional and can be used by connectors that require banking details
                                    </p>
                                </div>
